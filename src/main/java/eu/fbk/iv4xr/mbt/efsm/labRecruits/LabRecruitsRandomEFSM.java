...
/**
 * 
 * Generate a random EFSM that could be mapped into one or more Lab Recruits
 * level.
 * A level is defined by the number of its buttons and doors, but we also need a
 * way to
 * specify the number of rooms. In the current abstraction, a room has not a
 * direct representation
 * but it constitutes a strongly connected graph, where each node is button. Two
 * rooms can
 * be connected by one ore more doors, and a door can connect exactly two rooms
 * (including
 * the case of empty rooms, i.e., with no buttons).
 * <p>
 * We define the mean number of buttons M per room and we use binomial
 * distribution with mean M to generate
 * the number of buttons in each room. In this way the number of rooms is not
 * predefined but
...
public class LabRecruitsRandomEFSM {
	...
	public enum StateType {
		Button, GoalFlag, DoorSide, ColorScreen
	};
	...
	// LabRecruitsParameterGenerator parameterGenerator = new
	// LabRecruitsParameterGenerator();
	...
	public String getAnml() {
		...
		if (MBTProperties.MODELCRITERION[0] == ModelCriterion.STATE) {
			...
			// FIXME to update
			// throw new RuntimeException("PlanningBasedGenerator does not support
			// criterion: " + MBTProperties.MODELCRITERION[0]);
	...
	/**
	 * Return the model in ANML language. The domain modeling part is to be reused
	 * from the current manual implementation.
	 * 
	...
	public String getAnmlInstanceSingleGoal() {
		...
		for (Object t : efsm.getTransitons()) {
			...
			if (!trans.isSelfTransition() && trans.getGuard() == null) {
				anml.append("\nreachable(" + locationName(trans.getSrc().getId()) + ","
						+ locationName(trans.getTgt().getId()) + ") := true;");
			}
		}
		anml.append("\nreachable(*) := false;");
	...
	public static StateType getStateType(EFSMState eState) {
		...
		if (id.startsWith("cs")) {
			return StateType.ColorScreen;
		}

		if (id.startsWith("b") || id.startsWith("cb")) {
			return StateType.Button;
		}
	 ...
	 * 
	 * @return A room set represented as a list of vectors. Each vector represents
	 *         the buttons of a room.
	 */
	...
	/**
	 * Generate a set of rooms given the number of buttons and the mean number
	 * buttons in a room.
	 * 
	...
	private List<Vector<EFSMState>> generateRoomSetRoomDependent() {
		...
		while (availableButtons > 0) {
			...
			// int nRoomButtons = rndGenerator.nextBinomial((int)
			// Math.round(meanButtonsPerRoom/probabilityOfSuccess), probabilityOfSuccess);
			int nRoomButtons = (int) rndGenerator.nextPoisson(meanButtonsPerRoom) + 1;
	...
	private Pseudograph<Vector<EFSMState>, Integer> generatePlanarDoorsGraph(List<Vector<EFSMState>> roomSet) {
		...
		// rooms and initial doors to make the graph connected is the same as
		// generateDoorsGraph
		// need to reconcile

		// pick first node and update used and available rooms
		// Vector<?> firstNode = (Vector<?>)
		// availableRooms.toArray()[rndGenerator.nextInt(0, availableRooms.size()-1)];
		...
		// we would like to have at most 3 out nodes per vertex (i.e., 3 doors for a
		// room)
		/**
		 * need to check that while creating the connected graph we respect the 3 doors
		 * limit. Move above
		 */
		/**
		 * HashMap<Vector<LabRecruitsState>, Integer> roomsDoorsCount = new
		 * HashMap<Vector<LabRecruitsState>, Integer>();
		...
		// all rooms are used but there are still some available door: random add doors
		// between random rooms
		while (doorsGraph.edgeSet().size() < nDoors & completeGraph.edgeSet().size() > 0) {
			...
			// add to door graph
			doorsGraph.addEdge(completeGraph.getEdgeSource(newEdge), completeGraph.getEdgeTarget(newEdge),
					doorsGraph.edgeSet().size());

			// update number of doors per room count
			roomsDoorsCount.put(completeGraph.getEdgeSource(newEdge),
					roomsDoorsCount.get(completeGraph.getEdgeSource(newEdge)) + 1);
			roomsDoorsCount.put(completeGraph.getEdgeTarget(newEdge),
					roomsDoorsCount.get(completeGraph.getEdgeTarget(newEdge)) + 1);
			// if the new edge makes some room R having 3 doors, remove from complete graph
			// all edges with R
		...
		// all doors are used but there are still some room: merge remaining rooms with
		// used rooms
		// remove unused rooms
	 ...
	 * 
	 * HashMap<Integer, Set<EFSMState>> doorButtonsMap = new HashMap<Integer,
	 * Set<EFSMState>>();
	 * // iterator over doors graph vertex set (rooms)
	 * Iterator<Vector<EFSMState>> vertexIterator =
	 * doorsGraph.vertexSet().iterator();
	 * if (vertexIterator.hasNext()) {
	 * // pick initial room
	 * Vector<EFSMState> firstRoom = vertexIterator.next();
	 * // pick initial EFSM state from the first room
	 * EFSMState initialState = firstRoom.get(rndGenerator.nextInt(0,
	 * firstRoom.size() - 1));
	 * 
	 * // iterate over door graph and store door-button pairs while traveling
	 * BFSShortestPath<Vector<EFSMState>, Integer> shortestPath = new
	 * BFSShortestPath<Vector<EFSMState>, Integer>(
	 * doorsGraph);
	 * 
	 ...
	 * GraphPath<Vector<EFSMState>, Integer> path = shortestPath.getPath(firstRoom,
	 * nextRoom);
	 * ArrayList<Vector<EFSMState>> traversedRooms = (ArrayList<Vector<EFSMState>>)
	 * path.getVertexList();
	 ...
	 * .skip(rndGenerator.nextLong(0, doorsGraph.vertexSet().size() -
	 * 1)).iterator().next();
	 * EFSMState randomButtom = randomRoom.get(rndGenerator.nextInt(0,
	 * randomRoom.size() - 1));
	 * 
	 * Set<EFSMState> newSet = new HashSet<EFSMState>();
	 ...
	 * HashMap<EFSMState, Set<Integer>> buttonDoorsMap = new HashMap<EFSMState,
	 * Set<Integer>>();
	 * 
	 ...
	 * For each node of the doors graph create a totally connected graph of buttons.
	 * For each edge of
	 * the doors graph, create two connected door nodes in the EFMS. Each door node
	 * represents the side
	 * of a door, and has to be totally connected with the buttons in the respective
	 * room.
	 * 
	 * @return an EFSM
	 */
	private EFSM doorsGraphToEFSM() {
		...
		/*
		 * Operations: need to map a button to the doors it open to use the appropriate
		 * operation
		 * when creating transitions. This step also identify initial state.
		 * Starting from the door graph we build the context in such a way from a random
		 * starting room
		 * there is a path to each other rooms
		 */
		// HashMap<Integer, Set<EFSMState>> doorButtonsMap = new HashMap<Integer,
					...
					if (room.get(i).equals(room.get(j))) {
						if (getStateType(room.get(i)).equals(StateType.Button)) {
							...
							if (buttonDoorsMap.containsKey(room.get(i))) {
								...
								EFSMOperation op = new EFSMOperation(
										doorTriggers.toArray(new Assign[doorTriggers.size()]));
								toggle.setOp(op);
							}
						...
						// labRecruitsBuilder.withTransition(room.get(i), room.get(j), new
						// LabRecruitsFreeTravelTransition());
					}
				}
			}
		...
		// iterate over edges (doors) of the door graph and connect with buttons
		// connected subgraph
		...
		while (vertexIterator.hasNext()) {
			...
			// Set<EFSMTransition> outTransition =
			// labRecruitsBuilder.outgoingTransitionsOf(room.get(0));
			...
			for (int i = 0; i < localDoorList.size(); i++)
				for (int j = 0; j < localDoorList.size(); j++) {
					if (i != j) {
						...
						// EFSMTransition explore2 = new EFSMTransition();
						// explore2.setInParameter(inputParExplore);
						// labRecruitsBuilder.withTransition(localDoorList.get(j), localDoorList.get(i),
						// explore2);
					}
				}
		}
	 ...
	 * LabRecruitMutationManager csvManager = new
	 * LabRecruitMutationManager(csvLevel);
	...
	private String generateCSV() {
		...
		while (edgeIterator.hasNext()) {
			...
			// LabRecruitsDoor doorsGraphDoor =
			// doorButtonMap.getDoor("door"+doorsGraphEdge.toString());
			...
			Corridor corridor = Corridor.connect(csvRooms.get(GraphRoomToCsvRoom.get(source)),
					csvRooms.get(GraphRoomToCsvRoom.get(dest)));
	...
	public void saveEFSMtoDot(String scenarioID) throws IOException {
		if (efsm != null) {
			Function<EFSMState, String> stateLabeler = (state) -> {
				return (state.getId());
			};
			Function<EFSMTransition, String> edgeLabeler = (edge) -> {
				return ("");
			};
