...
public class ButtonDoors1FireWithDeath {
	...
	// death state
	public EFSMState death = new EFSMState("death");
	...
	public ButtonDoors1FireWithDeath(){
	}
	...
	public EFSM getModel() {
		...
		// die input parameter
		Var dieVar = new Var<LRActions>("die", LRActions.DIE);
		EFSMParameter inputParDie = new EFSMParameter(dieVar);
				
		// reset input parameter
		Var resetVar = new Var<LRActions>("reset", LRActions.RESET);
		EFSMParameter inputParReset = new EFSMParameter(resetVar);
		...
		// hp guard: not alive to move to death state
		EFSMGuard is_not_alive = new EFSMGuard(new BoolNot(is_hp_positive));
		...
		// reset hp
		Assign<Integer> hp_reset = new Assign(hp, new Const<Integer>(100));
		...
		// reset the HP
		EFSMOperation reset_game = new EFSMOperation(hp_reset);
		...
		EFSMTransition t_39 = new EFSMTransition();
		t_39.setGuard(is_not_alive);
		t_39.setInParameter(inputParDie);
		
		// reset transition
		EFSMTransition t_40 = new EFSMTransition();
		t_40.setGuard(is_not_alive);
		t_40.setInParameter(inputParReset);
		t_40.setOp(reset_game);
	    ...
	    buttonDoors1EFSM = buttonDoors1EFSMBuilder
	    		...
	    		// death and reset transtions
	    		.withTransition(fire, death, t_39)
	    		.withTransition(death, b_0, t_40)
